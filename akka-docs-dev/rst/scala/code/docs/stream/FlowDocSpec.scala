...
class FlowDocSpec extends AkkaSpec {
  ...
  "materialization in steps" in {
    ...
    val runnable: RunnableFlow[Future[Int]] = source.to(sink)
  ...
  "materializedMap is unique" in {
    ...
    // connect the Source to the Sink, obtaining a RunnableFlow
    val sink = Sink.fold[Int, Int](0)(_ + _)
    val runnable: RunnableFlow[Future[Int]] = Source(1 to 10).to(sink)
  ...
  "compound source cannot be used as key" in {
    ...
    val _ = timerMap.to(Sink.ignore) // WRONG: returned type is not the timers Cancellable!
    //#compound-source-is-not-keyed-run
  }
