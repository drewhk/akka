...
class TwitterStreamQuickstartDocSpec extends AkkaSpec {
  ...
  "simple broadcast" in {
    ...
    val g = FlowGraph() { implicit builder =>
      ...
      val b = Broadcast[Tweet](2)
      tweets ~> b.in
      b.out(0) ~> Flow[Tweet].map(_.author) ~> writeAuthors 
      b.out(1) ~> Flow[Tweet].mapConcat(_.hashtags.toList) ~> writeHashtags
    }
    g.run()
    //#flow-graph-broadcast
    // format: ON
  ...
  "count elements on finite stream" in {
    ...
    val counter: RunnableFlow[Future[Int]] = tweets.map(t => 1).to(sumSink)
